...
try:
    subprocess.run(
        ["python", "istaxcfg", "upload", "hpsctsnsw0", "4_r1.cfg"]
    )  # Set up the switch config for stream gate scenario 4
    ...
    for i in range(len(sourceList)):
        ...
        trafficItem.append(
            ixNetwork.Traffic.TrafficItem.add(
                ...
                Name="Traffic Item "
                + str(0),  # Want four of the same traffic item for scenario 4
        ...
        endpointSet = trafficItem[i].EndpointSet.add(
            Sources=tx_port[0].Protocols.find(), Destinations=rx_port3.Protocols.find()
        )  # Want four of the same traffic item for scenario 4
        ...
        VlanTagTemplate = ixNetwork.Traffic.ProtocolTemplate.find(
            DisplayName="^VLAN$"
        )  # template 71 is VLAN protocol, which matches traffic item with web UI
    ...
    for seq in range(
        2
    ):  # Use 2 passes for scenario 4; first passes cover tests 1,2 and 2nd for tests 3,4 with anamylzer in record mode
        ...
        for i in range(len(streamName)):
            ...
            if len(streamTrafficMembers[i]) == 0:
                ...
                for j in range(len(streamTrafficMembers[i])):
                    ...
                    VlanPriorityErrTot += (
                        VlanPriorityErr[streamTrafficMembers[i][j]]
                        - expectedPriority[streamTrafficMembers[i][j]]
                    )
                    print(
                        "VlanPriorityErr = ",
                        VlanPriorityErr,
                        "streamTrafficMembers[i][j] = ",
                        streamTrafficMembers[i][j],
                        "expectedPriority[streamTrafficMembers[i][j]] = ",
                        expectedPriority[streamTrafficMembers[i][j]],
                    )
        ...
        for m in range(len(sourceList)):
            ...
            if (4 * (seq + 1) + m) < len(expectedPriority):
                ...
                vlan_id_field.update(
                    ##SingleValue=i+4  # Set the VLAN priority to a different values
                    SingleValue=expectedPriority[
                        4 * (seq + 1) + m
                    ]  # Set the VLAN priority values to the secod set in expectedPriority
                )
                ##print("vlan_id_field = ", vlan_id_field)
