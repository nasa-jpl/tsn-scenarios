...
# from istaxcfgFunctions import istaxcfgFunc
import subprocess
...
try:
    subprocess.run(
        ["python", "istaxcfg", "upload", "hpsctsnsw0", "2_r1.cfg"]
    )  # Set up the switch config for stream gate scenario 2
    ...
    for seq in range(4):  # Use 4 passes for scenario 2
        if seq == 0:
            frameDelayCurr = [
                ...
                0,
                0,
            ]  # Populate first two values used to define traffic items, update the rest as per test cases
        ...
        print(
            "Current frameDelay = ",
            frameDelayCurr[2 * seq],
            ", ",
            frameDelayCurr[2 * seq + 1],
        )
